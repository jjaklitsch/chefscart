-- Complete Instacart Integration Schema Setup
...
-- Function to get Instacart health filters based on user preferences
CREATE OR REPLACE FUNCTION get_instacart_health_filters(
  user_dietary_restrictions TEXT[],
  user_organic_pref BOOLEAN DEFAULT FALSE
) RETURNS TEXT[] AS $$
DECLARE
  filters TEXT[] := ARRAY[]::TEXT[];
  restriction TEXT;
BEGIN
  -- Add organic if requested
  IF user_organic_pref THEN
    filters := filters || 'Organic';
  END IF;
  
  -- Map dietary restrictions
  FOREACH restriction IN ARRAY user_dietary_restrictions
  LOOP
    SELECT array_agg(instacart_filter) INTO filters
    FROM (
      SELECT DISTINCT unnest(filters || instacart_filter) as instacart_filter
      FROM health_filter_mapping 
      WHERE chefscart_preference = restriction AND is_active = TRUE
    ) subq;
  END LOOP;
  
  -- Remove nulls and duplicates
  SELECT array_agg(DISTINCT f) INTO filters
  FROM unnest(filters) f
  WHERE f IS NOT NULL;
  
  RETURN COALESCE(filters, ARRAY[]::TEXT[]);
END;
$$ LANGUAGE plpgsql;

-- Verify the setup with some test queries
-- SELECT * FROM ingredient_categories ORDER BY display_order;
-- SELECT * FROM health_filter_mapping WHERE is_active = true;
-- SELECT get_instacart_health_filters(ARRAY['organic', 'gluten-free'], true);