...
const STORAGE_KEY = 'chefscart_wizard_progress'

const saveToLocalStorage = (data: WizardProgress): void => {
  try {
...
const loadFromLocalStorage = (): WizardProgress | null => {
  try {
    if (typeof window !== 'undefined' && window.localStorage) {
...
const clearLocalStorage = (): void => {
  try {
    ...
    console.warn('Failed to clear wizard progress from localStorage:', error)
  }
}
...
export default function PreferencesChat({ onPreferencesComplete }: PreferencesChatProps) {
  ...
  useEffect(() => {
    ...
    if (savedProgress) {
      ...
      setPreferences(savedProgress.preferences)
      setSelectedOptions(savedProgress.selectedOptions)
      setMealConfiguration(savedProgress.mealConfiguration)
  ...
  }, [currentStep, preferences, selectedOptions, mealConfiguration, messages, userInput, isLoaded])
  ...
  const handleOptionSelect = (option: string) => {
    ...
    if (step && step.inputType === 'multiselect') {
  ...
  const handleSubmit = () => {
    ...
    if (!nextStepData) {
      console.error('No next step data found for step:', nextStep)
      return
    }
    ...
    const assistantMessage: ChatMessage = nextStepData.options
      ? { ...baseMessage, options: nextStepData.options }
      : baseMessage
                  ...
                  return (
                    <div className="space-y-4">
                      <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 sm:gap-3">
                        {currentStepData.options?.map((option) => (
                          <button
                            ...
                            className={`px-2 sm:px-4 py-2 sm:py-3 rounded-lg text-sm font-medium transition-colors border text-center leading-tight min-h-[44px] flex items-center justify-center ${
                      ...
                      <button
                        ...
                        className="w-full bg-orange-600 text-white py-3 px-4 rounded-lg font-medium hover:bg-orange-700 disabled:bg-gray-300 disabled:cursor-not-allowed transition-all duration-200 flex items-center justify-center shadow-sm hover:shadow-md disabled:opacity-50"
                      >
                ...
                if (currentStepData.inputType === 'mealconfig') {
                  ...
                  return (
                    <div className="space-y-6">
                      ...
                      <button
                        onClick={handleSubmit}
                        disabled={selectedMealTypes.length === 0 || Object.keys(mealConfiguration).length === 0}
                        className="w-full bg-orange-600 text-white py-3 px-4 rounded-lg font-medium hover:bg-orange-700 disabled:bg-gray-300 disabled:cursor-not-allowed transition-all duration-200 flex items-center justify-center shadow-sm hover:shadow-md disabled:opacity-50"
                      >
                        Continue <Send className="ml-2 h-4 w-4" />
                      </button>
