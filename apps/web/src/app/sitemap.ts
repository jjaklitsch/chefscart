...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  try {
    ...
    // Get all shop categories for shop category pages
    const { data: shopCategoriesData } = await supabase
      .from('shop_categories')
      .select('slug, updated_at')
      .order('sort_order')

    const shopCategoryPages = (shopCategoriesData || []).map((category: any) => ({
    ...
    // Get all cooking equipment for potential equipment-specific search pages
    const { data: equipmentData } = await supabase
      .from('cooking_equipment')
      .select('slug, display_name, updated_at')
      .eq('is_essential', true)
      .order('popularity_score', { ascending: false })
      .limit(50) // Limit to most popular items to avoid too many pages

    const equipmentSearchPages = (equipmentData || []).map((equipment: any) => ({
      url: `${baseUrl}/shop/search?q=${encodeURIComponent(equipment.display_name)}`,
      lastModified: equipment.updated_at ? new Date(equipment.updated_at) : new Date(),
      changeFrequency: 'monthly' as const,
      priority: 0.5,
    }))
